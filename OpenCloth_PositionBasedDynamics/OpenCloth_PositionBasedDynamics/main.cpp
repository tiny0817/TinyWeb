...
float kBend = 0.05f; //0.03以下效果较好
float kStretch = 0.9999f; 
float kDamp = 0.125f;//0.125越小越有弹性
glm::vec3 gravity=glm::vec3(0.0f,-0.0000981f,0.0f);  //0.00981 
...
void AddBendingConstraint(int pa, int pb, int pc, float k ) {
	...
	glm::vec3 np1 =X_bunny[c.p1]-X_bunny[c.p3];
	glm::vec3 np2 =X_bunny[c.p2]-X_bunny[c.p3];
...
void DrawGrid()
{
	...
	{
		glVertex3f((float)i,-0.5,(float)-GRID_SIZE);
		glVertex3f((float)i,-0.5,(float)GRID_SIZE);

		glVertex3f((float)-GRID_SIZE,-0.5,(float)i);
		glVertex3f((float)GRID_SIZE,-0.5,(float)i);
	}
	glEnd();
}
...
void UpdateBendingConstraint(int index) {
	...
	glm::vec3 p3 = tmp_X_bunny[c.p3];
	glm::vec3 p1 = tmp_X_bunny[c.p1]-p3;
	glm::vec3 p2 = tmp_X_bunny[c.p2]-p3;
	...
	glm::vec3 q1 = (n2-n1*d)/lenp1;
	//glm::vec3 q2 = (n2-n1*d)/lenp2;
	glm::vec3 q2 = (n1-n2*d)/lenp2;
	//glm::vec3 q3 = (n1-n2*d)/lenp3;
	glm::vec3 q3 =-q1-q2;
	//glm::vec3 q1 =-q2-q3;
...
void GroundCollision() //DevO: 24.07.2011
{
	for(size_t i=0;i<total_points_bunny;i++) {	
		if(tmp_X_bunny[i].y<-0.5) //collision with ground
		{
